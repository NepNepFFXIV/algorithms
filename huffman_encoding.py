...
class Node:
    __slots__=['character', 'frequency', 'left', 'right']
...
def create_huffman_tree(freqs):
    queue = []
    for char in freqs:
        heapq.heappush(queue, Node(char[0], char[1]))
...
def map_char_to_huffman_codes(root, string, huffman_map):
    if root.left is None and root.right is None:
        huffman_map[root.character] = string
    else:
        map_char_to_huffman_codes(root.left, string + LEFT, huffman_map)
        map_char_to_huffman_codes(root.right, string + RIGHT, huffman_map)

def encode(freqs, s):
    if len(freqs) <= 1: return None    
    huffman_tree = create_huffman_tree(freqs)
    huffman_map = {}
    map_char_to_huffman_codes(huffman_tree, "", huffman_map)
    result = []
    for character in s:
        result.append(huffman_map[character])
    return "".join(result)
...
def decode(freqs, bits):
    ...
    result = []
    for i in bits:       
        current_node = current_node.left if i == LEFT else current_node.right
        if current_node.left is None and current_node.right is None:
            result.append(current_node.character)
            current_node = huffman_tree
    return "".join(result)
...
'''
    ...
    c = encode(frequencies(string), string)
    d = decode(frequencies(string), c)
    # print(c)
    # print(d)
    print(d == string)